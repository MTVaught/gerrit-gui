...
const VOTE_VALUE = /(?:^|\s)(-)?Code-Review(?:([+-]\d+))?/

/**
 * The Code-Review vote the user cast on patch set `ps`, read from the change
 * messages the way Gerrit writes them: "Code-Review+1" for a vote, "-Code-Review"
 * when it was taken back. The last such message on the patch set wins. Null
 * when they replied on it without voting. For the current patch set the
 * labels are authoritative, so `current` overrides the messages when set.
 */
export function votedOnPatchSet(change: ChangeInfo, selfId: number, ps: number | null, current?: { patchSet: number; vote: number }): number | null {
  if (ps === null) return null
  if (current && current.patchSet === ps && current.vote !== 0) return current.vote
  let vote: number | null = null
  for (const m of change.messages ?? []) {
    if (m.author?._account_id !== selfId || m._revision_number !== ps) continue
    if (m.tag?.startsWith('autogenerated:')) continue
    const hit = VOTE_VALUE.exec(m.message)
    if (!hit) continue
    vote = hit[1] ? 0 : hit[2] !== undefined ? parseInt(hit[2], 10) : vote
  }
  return vote
}

/** The delta the fetch attached, when it is for the span the Review button opens; a stale one from another pair is dropped. */
export function sinceReview(change: ChangeInfo, last: number | null, patchSet: number): ReviewDelta | null {
  const d = change.review_delta
  if (!d || last === null || last >= patchSet) return null
  return d.basePatchSet === last && d.patchSet === patchSet ? d : null
}
...
export function classify(change: ChangeInfo, selfId: number, team: string[] = [], selfKeys: string[] = []): ChangeView {
  ...
  const lastReviewed = lastReviewedPatchSet(change, selfId)
  ...
  return {
    ...
    lastReviewedPatchSet: lastReviewed,
    lastReviewedVote: votedOnPatchSet(change, selfId, lastReviewed, { patchSet, vote: myVote }),
    sinceReview: sinceReview(change, lastReviewed, patchSet),
...
export const SORT_OPTIONS: { id: SortId; label: string; title: string }[] = [
  ...
  { id: 'new', label: 'Lines left to review, fewest first', title: 'Fewest lines you still have to read first: the whole change if you never reviewed it, only the lines since your last review if you did, none once you reviewed the current patch set' },
]
