 ...
 * message with no trailers at all says "no tags"; that turns solid red once
 * the change has reached Needs Review, since every change put up for review
 * is expected to carry at least one. While it is still in progress the
 * badge stays grey. Nothing when the message was not fetched.
 */
export function TagsButton(props: { message: string | undefined; state: ReviewState; small?: boolean }) {
  const { message } = props
  ...
  const missing = none && pastDraft(props.state)
  ...
  return (
    <span className="tags" ref={wrap}>
      <button
        className={'tagbtn' + (missing ? ' missing' : '') + (props.small ? ' sm' : '') + (open ? ' open' : '')}
